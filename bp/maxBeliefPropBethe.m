function [vals, belief_outer,iter_out] = ...
...
%                     end
                ...
                belief_inner{i} = belief_inner{i} / (sum(belief_inner{i})+eps);
...
%                     end
                    ...
                    Q_factor_joint{f} = Q_factor_joint{f} ./ (sum(Q_factor_joint{f}(:))+eps);
%                     sum_factor=sum_factor+max(Q_factor_joint{f}(:));
                end                
%                 [iter_in,iter_out,sum_factor]
            sum_belief_inner=sum_belief_inner+max(belief_inner{i}(:));
            end
            tmp_belief_inner(iter_out)=-log(sum_belief_inner);
            [iter_in,iter_out,-log(sum_belief_inner)]
            % check convergence
            converged = 1;
            maxdiff = 0;
            for i = 1:nvar
